...
const SchedulePreview = ({ generatedSchedule }) => {
    if (generatedSchedule.length === 0)
        return null;
    return (<div className="mt-8 bg-white border border-gray-200 rounded-lg p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-medium text-gray-800">Generated Timetable Preview</h2>
        <button className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          <lucide_react_1.Download className="w-4 h-4 inline mr-2"/>
          Export to PDF
        </button>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {generatedSchedule.map((slot) => (<tr key={slot.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-900">
                  <div className="space-y-1">
                    {slot.invigilators.map((invigilator, index) => (<div key={index} className="text-xs bg-gray-100 px-2 py-1 rounded">{invigilator}</div>))}
                  </div>
                </td>
              </tr>))}
          </tbody>
...
exports.default = SchedulePreview;
